...
define('URL_CSV', 'https://drive.google.com/uc?export=download&id=1MgPUgWyT-cSKAKmkzw6zh5efEX6JR-bq');
...
$colMap = [
  ...
  'GDCD'      => 9,
  'Ngoại ngữ' => 3,
  'LangCode'  => 10,  // Mã môn ngoại ngữ
];

// Danh sách môn 2018 để hiển thị trong form
$subjects2018 = [
  'Toán','Văn','Lí','Hóa','Sinh','Sử','Địa','GDCD','Ngoại ngữ'
];
    ...
    while (!$f->eof()) {
        ...
        if (trim($row[$colMap['SOBAODANH']]) === $targetSbd) {
            ...
            if ($chosen['m3'] === 'Ngoại ngữ') {
                $valLang = trim($row[$colMap['Ngoại ngữ']] ?? '');
                $code    = trim($row[$colMap['LangCode']]   ?? '');
                if (!is_numeric($valLang) 
                    || ($chosen['lang'] !== '' && $chosen['lang'] !== $code)
                ) {
                    unlink($tmp); return false;
                }
                $sum += (float)$valLang;
...
<body class="p-4">
  <h1 class="mb-4" style="text-align: center;"><b>TRA CỨU ĐIỂM VÀ XẾP HẠNG 2024</b></h1>
