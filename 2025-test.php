...
/**
 * 3) Lấy chi tiết điểm của SBD (tất cả các môn trong $colMap)
 */
function getFullScoresForSBD(string $url, string $targetSbd, array $colMap): array|false {
    $tmp = tempnam(sys_get_temp_dir(), 'csv');
    // <-- sửa ở đây
    if (!copy($url, $tmp)) {
        return false;
    ...
    while (!$f->eof()) {
        ...
        if (trim($row[$colMap['SOBAODANH']]) === $targetSbd) {
            $scores = [];
            foreach ($colMap as $subject => $col) {
                if (is_int($col)) {
                    $scores[$subject] = (isset($row[$col]) && is_numeric($row[$col]))
                                        ? (float)$row[$col]
                                        : null;
                }
            }
...
/**
 * Trả về 3 môn có tổng điểm lớn nhất và mã ngoại ngữ (nếu có).
 */
/**
 * Trả về 3 môn có tổng điểm lớn nhất và mã ngoại ngữ (nếu có).
 */
function pickTop3Subjects(array $scores): array {
    $valid = [];
    foreach ($scores as $subj => $score) {
        // Bỏ SOBAODANH và LangCode, chỉ lấy những môn có điểm số hợp lệ
        if ($subj !== 'SOBAODANH'
            && $subj !== 'LangCode'
            && is_numeric($score)
        ) {
            $valid[$subj] = (float)$score;
        }
    }
    arsort($valid);
    $top3 = array_slice($valid, 0, 3, true);
    $keys = array_keys($top3);

    // Xử lý mã ngoại ngữ
    $lang = in_array('Ngoại ngữ', $keys, true)
          ? ($scores['LangCode'] ?? '')
          : '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($sumSbd === false) {
    ...
    // 3) Lấy chi tiết điểm
    $fullScores = getFullScoresForSBD(URL_CSV, $sbd, $colMap);
    $result = [
      'SOBAODANH'=> $sbd,
      'sum'      => $sumSbd,
      'rank'     => $rt['rank'],
...
<?php if (!empty($result)): ?>
  ...
  <!-- In Số báo danh và tổng/rank như cũ -->
  <div class="alert alert-success text-center">
    <strong>Số báo danh:</strong> <?= htmlspecialchars($result['SOBAODANH']) ?>
    &nbsp;–&nbsp;
    <strong>Tổng điểm:</strong> <?= htmlspecialchars($result['sum']) ?>
    &nbsp;–&nbsp;
    <strong>Xếp hạng:</strong> <?= htmlspecialchars($result['rank']) ?>
    <?php if ($result['tie_count'] > 1): ?>
      <br><small>(Có <?= $result['tie_count'] ?> thí sinh cùng mức điểm)</small>
    <?php endif ?>
  </div>

  <?php
    $detailScores = array_filter(
      $result['scores'],
      fn($score, $subj) => $score !== null && $subj !== 'SOBAODANH',
      ARRAY_FILTER_USE_BOTH
    );
  ?>

  <?php if (!empty($detailScores)): ?>
  <div class="mb-5">
    <h4 class="text-center mb-3" style="color: #003366;"><b>Chi tiết điểm các môn</h4>
    <div class="table-responsive w-75 mx-auto">
      <table class="table table-bordered table-striped table-hover text-center">
        <thead class="table" style="background: #003366; color: #ffffff">
          <tr>
        ...
        <tbody>
        <?php foreach ($detailScores as $subj => $score): ?>
          <tr>
    ...
    <?php
  // Gợi ý tổ hợp từ fullScores (giữ $result['scores'] ban đầu)
  $suggest = pickTop3Subjects($result['scores']);
  // Khoi tao chuoi hien thi
  $combo = [$suggest['m1'], $suggest['m2'], $suggest['m3']];
  // Nếu có ngoại ngữ, thêm mã
  if ($suggest['lang'] !== '') {
      $combo[2] .= " ({$suggest['lang']})";
  }
?>
<div class="alert alert-secondary text-center w-75 mx-auto">
  <strong>Gợi ý tổ hợp môn:</strong>
  <?= implode(' – ', array_map('htmlspecialchars', $combo)) ?>
</div>
  </div>
  <?php endif ?>
...
<?php elseif ($_SERVER['REQUEST_METHOD'] === 'POST'): ?>
  <div class="alert alert-danger text-center">
    Không tìm thấy dữ liệu Số báo danh trong tổ hợp môn đã chọn.
  </div>
<?php endif ?>
